...
@Component("redisConfigService")
public class RedisConfigServiceImpl implements InitializingBean, RedisConfigService {

	private final Logger logger = LoggerFactory.getLogger(RedisConfigServiceImpl.class);
		...
		public void run() {
			while (true) {
				try {
					...
					logger.error("load redis.properties error:", e);
				}
				//
				try {
					Thread.sleep(30000L);
