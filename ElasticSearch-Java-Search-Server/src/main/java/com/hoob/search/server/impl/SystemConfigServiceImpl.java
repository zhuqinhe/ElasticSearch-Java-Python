...
public class SystemConfigServiceImpl implements InitializingBean, SystemConfigService {

	private final Logger logger = LoggerFactory.getLogger(SystemConfigServiceImpl.class);
		...
		public void run() {
			while (true) {
				try {
					...
					logger.error("load system.properties error:", e);
				}
				//
				try {
					Thread.sleep(30000L);
